...
/** @typedef {import('../driver.js')} LegacyDriver */
/** @typedef {import('./target-manager.js')} TargetManager */
...
class NetworkMonitor {
  ...
  /** @param {TargetManager|LegacyDriver} targetManagerOrLegacyDriver */
  constructor(targetManagerOrLegacyDriver) {
    // TODO(FR-COMPAT): remove legacy driver.
    /** @type {TargetManager|LegacyDriver} */
    this._targetManagerOrLegacyDriver = targetManagerOrLegacyDriver;

    /** @type {LH.Gatherer.FRProtocolSession} */
    // @ts-expect-error - legacy driver fallback
    this._session = targetManagerOrLegacyDriver.defaultSession ??
        // @ts-expect-error
        targetManagerOrLegacyDriver.rootSession();
  ...
  async enable() {
    ...
    // @ts-expect-error
    const isLegacyRunner = Boolean(this._targetManagerOrLegacyDriver._domainEnabledCounts);

    if (isLegacyRunner) {
      /** @type {LegacyDriver} */
      // @ts-expect-error
      const legacyDriver = this._targetManagerOrLegacyDriver;
      legacyDriver.addProtocolMessageListener(this._onProtocolMessage);
    } else {
      /** @type {TargetManager} */
      // @ts-expect-error
      const targetManager = this._targetManagerOrLegacyDriver;
      targetManager.on('protocolevent', this._onProtocolMessage);
    }
  }
  ...
  async disable() {
    ...
    if (isLegacyRunner) {
      ...
      // @ts-expect-error
      const legacyDriver = this._targetManagerOrLegacyDriver;
      legacyDriver.removeProtocolMessageListener(this._onProtocolMessage);
    } else {
      /** @type {TargetManager} */
      // @ts-expect-error
      const targetManager = this._targetManagerOrLegacyDriver;
      targetManager.off('protocolevent', this._onProtocolMessage);
    }
